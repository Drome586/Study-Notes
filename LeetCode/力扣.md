...
​	<img src="力扣.assets/image-20230802162310586.png" alt="image-20230802162310586" style="zoom:80%;" />
...
​	<img src="力扣.assets/image-20230804092433627.png" alt="image-20230804092433627" style="zoom:80%;" />
...
​	<img src="力扣.assets/image-20230808151338353.png" alt="image-20230808151338353" style="zoom:80%;" />
...
​	<img src="力扣.assets/image-20230810105436698.png" alt="image-20230810105436698" style="zoom:80%;" />
...
​	<img src="力扣.assets/image-20230810112706276.png" alt="image-20230810112706276" style="zoom:80%;" />
...
​	<img src="力扣.assets/image-20230815103417358.png" alt="image-20230815103417358" style="zoom:80%;" />
...
​	<img src="力扣.assets/image-20230817224553827.png" alt="image-20230817224553827" style="zoom:80%;" />

```java
//1.	保证二叉树是高度平衡的。用到二分搜索。
travel(int[] nums,0,nums.length-1){
...
//完全背包  都是正序遍历，但是先遍历物品在遍历背包求的是组合数，先遍历背包再遍历物品求的是排列数。
//1.先遍历物品，求排列
for(int i = 0;i < weight.length;i++){
...
​	<img src="力扣.assets/image-20230818101358723.png" alt="image-20230818101358723" style="zoom:80%;" />

```java
//1.	直接暴力枚举，一个数字拆分，不可能拆分出数字比他还大的，假如拆分4，就相当于从1，2，3里面找组合。
dp[1] = 1;
dp[2] = 1;
for(int i = 3;i < n;i++){
    for(int j = 1;j < i;j++){
...
​	![image-20230818154512410](力扣.assets/image-20230818154512410.png)
...
​	<img src="力扣.assets/image-20230819145817255.png" alt="image-20230819145817255" style="zoom:80%;" />
...
​	<img src="力扣.assets/image-20230820095430871.png" alt="image-20230820095430871" style="zoom:80%;" />

```java
//1.	想到一个字符串，判断i+1跟j-1截取的字符串是不是很重要。
//2。	定义的是一个boolean数组；上述1的情况，如果说相等，那么就判断dp[i+1]跟dp[j-1]是否是true就可以。
//3.	既然是i+1跟j-1，那么想象一下二维数组中的情况，从左下角推出右上角的ij，所以i应该是从下往上，j从左往右。

for(int i = len-1;i >= 0;i--){
    for(int j = i;j < len;j++){
        if(s.charAt(i) == s.charAt(j)){
            //判断两个字符或者一个字符的情况，这种如果相等的话，那么都是true；
            if(j-i<=1){
                dp[i][j] = true;
                result++;
            }else if(dp[i+1][j-1]){
...
​	<img src="力扣.assets/image-20230806220118878.png" alt="image-20230806220118878" style="zoom:80%;" />
